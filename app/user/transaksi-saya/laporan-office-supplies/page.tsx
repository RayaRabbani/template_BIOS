...
import DetailApprovalModal from '@/components/modals/user/office-supplies/detail-approval-modal';
import PendingApprovalModal from '@/components/modals/user/office-supplies/pending-approval-modal';
...
export default function HistoryPage() {
  ...
  const fetchTransaksi = useCallback(
    async (
      ...
      try {
        ...
        const list = ((json.data ?? []) as ApiTransaksi[])
          .filter(
            (it: ApiTransaksi) =>
  ...
  }, [search, filterStatus, filterType, appliedPeriod, page]);
              ...
              <div className="flex items-center justify-center p-12">
                <div className="text-center">
                  ...
                  <p className="mt-2 text-sm text-neutral-500">
                    Tidak ada transaksi office supplies untuk periode ini.
                  </p>
                </div>
              </div>
            ) : (
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {paginated.map((item, idx) => (
                        ...
                        >
                          {visibleCols.type && (
                            <TableCell className="pl-6 text-sm font-medium capitalize">
                              {item.type}
                            </TableCell>
                          )}
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Button
                                    ...
                                    onClick={() =>
                                      router.push(
                                        `/user/transaksi-saya/laporan-office-supplies/detail/${encodeURIComponent(
                      ...
                      {(() => {
                        ...
                        return firstAsset ? (
                          <div className="h-16 w-16 flex-shrink-0 cursor-zoom-in overflow-hidden rounded-lg border border-neutral-200 bg-white dark:border-neutral-700">
                            <Image
                              src={getValidImageUrl(firstAsset.pic)}
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="line-clamp-2 font-semibold text-neutral-900 dark:text-neutral-100">
                          {(() => {
                            const assets = getAssetsForItem(item);
                            if (assets.length === 0)
                              return 'Nama asset tidak tersedia';
                            const names = assets
                              .map((a: AssetItem) => a.nama)
                              .filter(Boolean);
                            return names.length > 0
                              ? names.join(', ')
                              : 'Nama asset tidak tersedia';
                          })()}
                        </p>
                      </div>
                    </div>
