...
import {
  approveOfficeSuppliesRequest,
  getOfficeSuppliesApprovals,
  rejectOfficeSuppliesRequest,
} from '@/lib/api/approver';
import { resolveAssetImage } from '@/lib/images';
import { cn } from '@/lib/utils';
import type { HistoryItem } from '@/types/approver/office-supplies';
...
function getValidImageUrl(url: string | null | undefined): string {
  ...
  const resolvedUrl = resolveAssetImage(url, 'asset');
...
export default function HistoryPage() {
  ...
  useEffect(() => {
    async function loadOfficeSuppliesData() {
      setLoading(true);
      try {
        const options: {
          search?: string;
        ...
        const json = await getOfficeSuppliesApprovals(session.user.id, options);
        ...
        const mapped: HistoryItem[] = (json.data ?? []).map(
          (d: Record<string, unknown>) => {
            ...
            return {
              ...
              date: String(d.tgl_permintaan ?? d.created_at ?? ''),
              tgl_permintaan: (d.tgl_permintaan as string | null) ?? null,
              employee: String(d.nama ?? d.nama_pengaju ?? '-'),
              no_badge: (d.no_badge as string | null) ?? null,
              pic: (d.pic as string | null) ?? null,
              qty: firstItem?.qty ?? (d.qty as number) ?? 1,
              status,
              assetName: firstItem?.nama ?? undefined,
              assetImage:
                resolveAssetImage(firstItem?.pic ?? d.pic ?? null, 'office') ??
                undefined,
              // Add approver data mapping
              ...
              item: Array.isArray(d.item)
                ? d.item.map((it: Record<string, unknown>) => ({
                    ...
                    nama: it.nama as string | undefined,
                    pic: resolveAssetImage(it.pic as string | null, 'office'),
                    qty: it.qty as number | undefined,
                  }))
                : undefined,
            } as HistoryItem;
          }
        ...
        console.error('Failed to load office supplies approvals:', error);
        setData([]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <>
      <Navbar
        title="Approval Permintaan Office Supplies"
        showSearch={false}
        showCart={false}
        rightAction={
          <ApproverBell
              ...
              <div className="flex items-center justify-center p-12">
                <div className="text-center">
                  ...
                  <p className="mt-2 text-sm text-neutral-500">
                    Tidak ada pengajuan permintaan office supplies untuk periode
                    ini.
                  </p>
                </div>
              </div>
                  ...
                  <Table>
                    <TableHeader className="bg-neutral-100/60 dark:bg-neutral-800/40">
                      <TableRow>
                        ...
                        <TableHead className="font-semibold text-neutral-700 dark:text-neutral-300">
                          <div className="flex items-center gap-1">Pengaju</div>
                        </TableHead>
