...
import {
  approvePermintaanRequest,
  getPermintaanApprovals,
  rejectPermintaanRequest,
...
import type {
  ...
  ApiTransaksi,
  HistoryItem,
} from '@/types/approver/permintaan';
...
export default function HistoryPage() {
  ...
  useEffect(() => {
    async function loadApproverData() {
      ...
      try {
        ...
        const json = (await getPermintaanApprovals(
          session.user.id,
          options
        )) as ApiResponse;
        ...
        const mapped: HistoryItem[] = (json.data ?? []).map(
          (d: ApiTransaksi) => {
            ...
            return {
              ...
              date: String(d.tgl_permintaan ?? d.tgl_pinjam ?? ''),
              tgl_permintaan: d.tgl_permintaan ?? null,
  ...
  async function performConfirm(note?: string) {
    ...
    try {
      ...
      if (currentAction === 'approve') {
        apiResult = await approvePermintaanRequest(confirmTargetId, payload);
        ...
        apiResult = await rejectPermintaanRequest(confirmTargetId, payload);
        if (apiResult.statusCode === 200 || apiResult.statusCode === 201) {
          setData(d =>
            d.map(it =>
              it.id === confirmTargetId
  ...
  return (
    <>
      <Navbar
        title="Approval Permintaan Asset"
        showSearch={false}
        showCart={false}
        rightAction={
          <ApproverBell
              ...
              <div className="flex items-center justify-center p-12">
                <div className="text-center">
                  ...
                  <p className="mt-2 text-sm text-neutral-500">
                    Tidak ada pengajuan permintaan untuk periode ini.
                  </p>
                </div>
              </div>
            ) : (
