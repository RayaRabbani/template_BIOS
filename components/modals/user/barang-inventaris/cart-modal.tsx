...
import type { ApiCartItem } from '@/types/user/barang-inventaris/modals';
  ...
  const fetchCart = useCallback(async () => {
    try {
      const json = isPermintaan
        ...
        : await getBorrowingCart(NO_BADGE);
      const data = json?.data;
      if (Array.isArray(data)) {
        const mapped: CartItem[] = data.map((it: ApiCartItem, idx: number) => {
          const id = Number(it.id ?? idx);
          ...
          const image = resolveAssetImage(gambar, 'office') ?? undefined;
  ...
  const saveChanges = async () => {
    ...
    try {
      ...
      for (const u of updates) {
        try {
          const orig = originalItems.find(o => o.id === u.id);
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={v => setOpen(v)}>
        <DialogContent
          className={cn(
            'rounded-sm border bg-white shadow-xl dark:bg-neutral-900',
        ...
        >
          <DialogHeader className="-mt-2">
            <DialogTitle className="text-xl font-semibold text-neutral-900 dark:text-neutral-100">
              {isPermintaan ? 'Detail Permintaan' : 'Detail Peminjaman'}
            </DialogTitle>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {isPermintaan
                ? 'Berikut detail permintaan barang inventaris anda.'
                : 'Berikut detail peminjaman barang inventaris anda.'}
            </p>
          </DialogHeader>
              ...
              {items.map((item: CartItem) => (
                ...
                >
                  <div className="flex items-start gap-4">
                    <div
                      className="h-16 w-16 flex-shrink-0 cursor-zoom-in overflow-hidden rounded-sm bg-gray-100 dark:bg-neutral-700"
                      onClick={() => {
                        setPreviewImage(
                    ...
                    <div className="max-w-[55vw] min-w-0 sm:max-w-none">
                      <p className="text-sm leading-snug font-medium text-neutral-900 sm:text-base dark:text-neutral-100">
                        {String(item.name)}
                      </p>
                  ...
                  <div className="flex flex-shrink-0 items-center gap-3">
                    {isPermintaan ? (
                      <div className="flex items-center gap-1 rounded-full border border-neutral-200 bg-white px-2 py-0.5 sm:gap-2 sm:px-3 sm:py-1 dark:border-neutral-600 dark:bg-neutral-700">
                        <Button
                          variant="ghost"
                          size="icon"
                        ...
                        >
                          <Minus className="h-3 w-3 text-neutral-700 sm:h-4 sm:w-4 dark:text-neutral-200" />
                        </Button>
                        <span className="text-xs text-neutral-800 sm:text-sm dark:text-neutral-100">
                          {String(item.qty)}
                        </span>
                        <Button
                          variant="ghost"
                          ...
                          className="h-4 w-4 cursor-pointer rounded-full hover:bg-gray-200 sm:h-5 sm:w-5 dark:hover:bg-neutral-600"
                        >
                          <Plus className="h-3 w-3 text-neutral-700 sm:h-4 sm:w-4 dark:text-neutral-200" />
                        </Button>
                      </div>
                    ) : null}
                    <Button
                      ...
                      className="h-6 w-6 flex-shrink-0 cursor-pointer rounded-full bg-red-100 hover:bg-red-200 sm:h-9 sm:w-9 dark:bg-red-900/30 dark:hover:bg-red-900/50"
                    >
                      <Trash className="h-4 w-4 text-red-600 sm:h-4 sm:w-4 dark:text-red-300" />
                    </Button>
                  </div>
                </div>
              ))}
          ...
          {items.length > 0 && (
            <div className="mt-3 flex justify-end">
              ...
              >
                ...
                {isPermintaan ? 'Permintaan' : 'Pinjam'}
              </Button>
            </div>
          )}
        </DialogContent>
      ...
      <Dialog open={openPreview} onOpenChange={setOpenPreview}>
        <DialogContent
          ...
          className="max-w-4xl border-none bg-transparent p-0 shadow-none [&>button]:hidden"
        >
          <VisuallyHidden>
            <DialogTitle>Preview Image</DialogTitle>
          </VisuallyHidden>
          <div className="relative flex h-[80vh] w-full items-center justify-center bg-transparent">
            {previewImage && (
              <Image
                src={previewImage}
                alt="Preview"
