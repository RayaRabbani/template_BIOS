...
import {
  cancelPeminjamanRequest,
  cancelPermintaanRequest,
  getPendingTransaksi,
} from '@/lib/api/user';
...
import type {
  ...
  DetailApprovalData,
  PendingApiResponse,
} from '@/types/user/barang-inventaris/modals';

import DetailApprovalModal from '../barang-inventaris/detail-approver-modal';
...
export default function PendingApprovalModal({
  ...
  useEffect(() => {
    async function fetchPending() {
      ...
      try {
        ...
        const pinjamArray = payload?.pinjamResult ?? json?.pinjamResult ?? [];
        const permintaanArray =
          payload?.permintaanResult ?? json?.permintaanResult ?? [];

        const pinjamPendingCount = Array.isArray(pinjamArray)
          ? pinjamArray.filter(it => {
        ...
        const permintaanPendingCount = Array.isArray(permintaanArray)
          ? permintaanArray.filter(it => {
              const s = (it?.status || '').toString().toLowerCase();
              return (
                s.includes('diajukan') ||
                s.includes('menunggu') ||
        ...
        setPinjamCount(Number(pinjamPendingCount) || 0);
        setPermintaanCount(Number(permintaanPendingCount) || 0);
      } catch (err) {
        setError(String(err));
      } finally {
        setLoading(false);
  ...
  const performCancelConfirmed = async (
    id: string | number,
    type: 'peminjaman' | 'permintaan'
  ) => {
    try {
      if (type === 'peminjaman') {
        await cancelPeminjamanRequest(id);
      } else {
        await cancelPermintaanRequest(id);
      }
      ...
      const payload: ApiResponseData | null =
        (json?.data as ApiResponseData) ?? null;
      setData(payload);
      // recompute pending counts from returned payload
      const pinjamArray = payload?.pinjamResult ?? json?.pinjamResult ?? [];
      const permintaanArray =
        payload?.permintaanResult ?? json?.permintaanResult ?? [];
      ...
      <DialogContent className="w-full max-w-xl overflow-hidden rounded-md border border-neutral-200 bg-white p-0 shadow-[0_12px_45px_-10px_rgba(0,0,0,0.2)] dark:border-neutral-800 dark:bg-neutral-900">
        <div className="p-6">
          ...
          <Tabs
            value={tab}
            onValueChange={value =>
              setTab(value as 'peminjaman' | 'permintaan')
            }
          >
            <TabsList className="h-auto w-full bg-neutral-100 p-0 dark:bg-neutral-800">
                ...
                .map(t => (
                  <TabsTrigger
                    ...
                    className={cn(
                      ...
                      'hover:data-[state=inactive]:bg-transparent hover:data-[state=inactive]:text-neutral-600 hover:data-[state=inactive]:dark:bg-transparent hover:data-[state=inactive]:dark:text-neutral-300'
                    )}
                  >
            ...
            <TabsContent value="peminjaman" className="mt-2">
              <ScrollArea
                className={cn(
                  currentList.length > 3 && tab === 'peminjaman'
                    ? 'h-[280px]'
                    : 'h-auto'
                )}
              >
                              ...
                              <div className="flex gap-2">
                                {item.status === 'Diajukan' && (
                                  <Button
                                    ...
                                    onClick={() =>
                                      handleCancel(item.id, 'peminjaman')
                                    }
                                  >
                                    <X
                                      size={16}
                                  ...
                                  onClick={() => {
                                    ...
                                    const mapped = {
                                      ...
                                      assetImage: firstItem
                                        ...
                                        : undefined,
                                      requestDate:
                                        item.tgl_peminjaman ?? undefined,
                                      tgl_permintaan:
                                        item.tgl_permintaan ?? undefined,
                                      approveDate:
                                        item.approval_at ?? undefined,
                                      returnDate:
                                        item.tgl_pengembalian ?? undefined,
                                      status:
            ...
            <TabsContent value="permintaan" className="mt-2">
              <ScrollArea
                className={cn(
                  currentList.length > 3 && tab === 'permintaan'
                    ? 'h-[280px]'
                    : 'h-auto'
                )}
              >
                  ...
                  <AnimatePresence mode="wait">
                    ...
                    >
                      ...
                      {!loading &&
                        !error &&
                        (tab === 'permintaan' &&
                        (data?.permintaanResult ?? []).length === 0 ? (
                          <div className="flex flex-col items-center justify-center p-8 text-center text-neutral-500">
                            <Inbox className="mb-3 h-14 w-14 text-neutral-400" />
                            <p className="font-medium">
                              Tidak ada pengajuan yang menunggu
                              ...
                              <div className="flex gap-2">
                                {item.status === 'Diajukan' && (
                                  <Button
                                    ...
                                    onClick={() =>
                                      handleCancel(item.id, 'permintaan')
                                    }
                                  >
                                    <X
                                      size={16}
